...
function hydrateRequest(context, request) {
    ...
    hydrated.className = helpers_1.ensureSuffix(helpers_1.pascalCase(request.name), helpers_1.upperCaseFirst(suffix));
    hydrated.fileName = helpers_1.removeSuffix(helpers_1.paramCase(request.name), "-" + helpers_1.paramCase(suffix));
    if (request.type === 'pipe')
        hydrated.pipeName = helpers_1.camelCase(request.name);
...
function applyTemplates(request, templates) {
    ...
    templates.forEach(function (fileContent, filePath) {
        ...
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.PIPENAME_VARIABLE, request.pipeName);
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.IMPORTSTATEMENT_VARIABLE, request.importStatement);
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.IONICPAGE_VARIABLE, request.ionicPage);
        ...
        fileContent = helpers_1.replaceAll(fileContent, GeneratorConstants.TABS_IMPORTSTATEMENT_VARIABLE, request.tabsImportStatement);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    envVar = helpers_1.getStringPropertyValue("IONIC_" + hydratedRequest.type.toUpperCase() + "S_NG_MODULE_PATH");
                ...
                case 1:
                    ...
                    typescriptFilePath = helpers_1.changeExtension(templatesArray.filter(function (path) { return path_1.extname(path) === '.ts'; })[0], '');
                    helpers_1.readFileAsync(ngModulePath).then(function (content) {
                        ...
                        return helpers_1.writeFileAsync(ngModulePath, content);
                    });
                    return [2 /*return*/];
            }
        });
...
function tabsModuleManipulation(tabs, hydratedRequest, tabHydratedRequests) {
    ...
    if (!ngModulePath) {
        ...
        return helpers_1.readFileAsync(tabsPath_1).then(function (content) {
            tabHydratedRequests.forEach(function (tabRequest) {
                var typescriptFilePath = helpers_1.changeExtension(tabRequest.generatedFileNames.filter(function (path) { return path_1.extname(path) === '.ts'; })[0], '');
            ...
            return helpers_1.writeFileAsync(tabsPath_1, modifiedContent_1);
        });
    }
}
...
function createConstStatments(pageConstantFile, request) {
    helpers_1.readFileAsync(pageConstantFile).then(function (content) {
        content += "\nexport const " + helpers_1.constantCase(request.className) + " = '" + request.className + "';";
        helpers_1.writeFileAsync(pageConstantFile, content);
    });
}
exports.createConstStatments = createConstStatments;
function createPageConstants(context) {
    var pageConstantFile = path_1.join(context.pagesDir, 'pages.constants.ts');
    helpers_1.writeFileAsync(pageConstantFile, '//Constants for getting type references');
}
